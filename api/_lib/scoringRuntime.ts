...
export function scoreLeadWithConfig(input: ScoreInput, config: ScoringConfig) {
  ...
  contributions.push({ key: "demo_requested", label: "Demo requested", value: 0 });
  contributions.push({ key: "industry_match", label: "Industry match", value: config.weights.fit.industryMatch });
  contributions.push({ key: "company_size_fit", label: "Company size fit", value: config.weights.fit.companySizeFit });
  contributions.push({ key: "budget_fit", label: "Budget fit", value: config.weights.fit.budgetFit });
  ...
  contributions.push({ key: "recency", label: "Recent activity", value: recency });

  const sourcePrior = config.weights.sourcePrior[input.source.toLowerCase()] ?? 0;
  contributions.push({ key: "source_prior", label: "Source quality", value: sourcePrior });
