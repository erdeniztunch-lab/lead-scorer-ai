...
export function HeroQueuePreview() {
  ...
  return (
    <aside className="glass-panel-strong accent-halo relative rounded-3xl p-5 md:p-6">
      ...
      <div className="relative z-10">
        ...
        <div className="mt-4 grid gap-2 sm:grid-cols-3">
          {queueSnapshot.summary.map((item) => (
            <div key={item.label} className="glass-chip rounded-xl p-3">
        ...
        <div className="mt-4 space-y-2">
          {queueSnapshot.leads.map((lead, index) => (
            <div key={lead.name} className="glass-chip flex items-center gap-3 rounded-xl px-3 py-2">
              <span className="w-4 text-xs text-muted-foreground">#{index + 1}</span>
              <div className="min-w-0 flex-1">
                <p className="truncate text-sm font-medium">{lead.name}</p>
                <p className="truncate text-xs text-muted-foreground">{lead.company}</p>
              </div>
              <span className="glass-chip hidden rounded-full px-2 py-0.5 text-[10px] text-secondary-foreground sm:inline-flex">
                {lead.reason}
              </span>
        ...
        <div className="glass-chip mt-4 flex flex-wrap items-center justify-between gap-2 rounded-xl px-3 py-2">
